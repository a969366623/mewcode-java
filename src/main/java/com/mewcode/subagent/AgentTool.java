...
public class AgentTool implements Tool {
    ...
    /**
     * 关掉 fork 后，省略 subagent_type 不再 fork，而是回退到通用 agent。
     * 用「关闭」而不是「开启」语义，是为了让默认值就是默认行为（fork 可用），
     * 每个构造点不必都显式赋值。
     */
    ...
    public Map<String, Object> schema() {
        ...
        properties.put("plan_mode_required", Map.of(
                "type", "boolean",
                "description",
                "Only meaningful together with team_name. When true, the teammate starts in plan mode: "
                        + "it can read and investigate but cannot modify anything until it submits a plan "
                        + "and you approve it via SendMessage with type='plan_approval_response'. Use it for "
                        + "risky or ambiguous tasks where a wrong direction would cost a lot of rework."));
    ...
    public ToolResult execute(Map<String, Object> args) {
        ...
        boolean planModeRequired = args.get("plan_mode_required") instanceof Boolean pmr && pmr;
        ...
        if (teamName != null && !teamName.isEmpty() && teamManager != null) {
            ...
            return runAsTeammate(spec, teamName, description, prompt, modelOverride, isolation, planModeRequired);
        }
    ...
    private ToolResult runAsTeammate(SubAgentSpec spec, String teamName,
                                     String description, String prompt, String modelOverride,
                                     String isolation, boolean planModeRequired) {
        // 团队不存在就顺手建一个：coordinator 模式下 TeamCreate 不在白名单里，
        // 要求 Lead 先建团队再派人，它会卡在第一步。
        var team = teamManager.getTeam(teamName);
        if (team == null) {
            team = teamManager.createTeam(teamName, com.mewcode.teams.TeamManager.detectBackend(),
                    com.mewcode.teams.TeammateRunner.LEAD_NAME, description);
        }
        ...
        // 团队共享任务板工具：队友之间共享同一份任务列表（覆盖个人版同名工具）
        subRegistry.register(new com.mewcode.teams.TeamTaskTools.TaskCreateTool(teamManager, teamName, memberName));
        subRegistry.register(new com.mewcode.teams.TeamTaskTools.TaskGetTool(teamManager, teamName));
        subRegistry.register(new com.mewcode.teams.TeamTaskTools.TaskListTool(teamManager, teamName));
        subRegistry.register(new com.mewcode.teams.TeamTaskTools.TaskUpdateTool(teamManager, teamName));
        ...
        // 补齐成员元信息，让 config.json 里带上 agent 类型、模型和 worktree 路径
        team.setMemberMeta(memberName, spec == null ? null : spec.name(), modelOverride, workdir);
        ...
        try {
            var spawnResult = com.mewcode.teams.SpawnDispatcher.spawnTeammate(
                    new com.mewcode.teams.SpawnDispatcher.SpawnConfig(
                            ...
                            teammateChecker(planModeRequired)));
     ...
     */
    private com.mewcode.permission.PermissionChecker teammateChecker(boolean planModeRequired) {
        if (!planModeRequired) {
            // 普通队友不带检查器，沿用既有行为
            return null;
        }
        var root = java.nio.file.Path.of(System.getProperty("user.dir"));
        return new com.mewcode.permission.PermissionChecker(
                com.mewcode.permission.PermissionMode.PLAN, root);
    }
